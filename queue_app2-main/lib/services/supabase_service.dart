...
class SupabaseService {
  ...
  Future<List<QueueEntry>> getQueueEntriesByDepartment(
    ...
    try {
      final response = await _supabase
          ...
          .order(SupabaseConfig.batchNumberColumn, ascending: true) // Older batches first
          .order('is_priority', ascending: false) // Priority users first within batch
          .order(
  ...
  Future<QueueEntry?> addQueueEntry({
    ...
    String userType = 'Student', // Changed from studentType to userType (Student, Graduated, or External)
    String? gender,
    int? age,
    int? graduationYear,
  }) async {
    try {
      // Check if purpose exists, if not create it (for custom purposes)
      Purpose? existingPurpose = await getPurposeByName(purpose);
      if (existingPurpose == null) {
        // Purpose doesn't exist, try to create it automatically
        print('⚠️ Purpose "$purpose" not found, attempting to create it...');
        try {
          existingPurpose = await addPurpose(
            name: purpose,
            description: 'Custom purpose: $purpose',
          );
          print('✅ Successfully created purpose "$purpose"');
        } catch (createError) {
          print('❌ Failed to create purpose "$purpose": $createError');
          // If creation fails (e.g., permission denied), throw a user-friendly error
          throw Exception('The purpose "$purpose" is not recognized and could not be added automatically.\n\n'
              'Please select a purpose from the list or contact an administrator to add this purpose.');
        }
      }

      // Get the next global queue number (continuous across all departments)
      ...
      final entry = QueueEntry(
        ...
        batchNumber: currentBatch, // Assign current batch number
        isPwd: isPwd,
        isSenior: isSenior,
        isPregnant: isPregnant,
        userType: userType, // Changed from studentType to userType
      ...
      if (e.toString().contains('violates foreign key constraint')) {
        if (e.toString().contains('fk_queue_entries_purpose')) {
          print('💡 Foreign Key Error: Purpose "$purpose" does not exist in purposes table');
          print('   Custom purposes must be added to the purposes table first');
          // Re-throw with a more user-friendly error message
          throw Exception('The purpose "$purpose" is not recognized. Please select a valid purpose from the list or contact an administrator to add this purpose.');
        } else if (e.toString().contains('fk_queue_entries_department')) {
          print('💡 Foreign Key Error: Department "$department" does not exist');
          throw Exception('The department "$department" is not recognized. Please contact an administrator.');
        }
      }
  ...
  // Reset queue / Cut-Off function
  // This increments the batch number instead of deleting entries
  // Old entries keep their queue numbers and batch numbers
  // New entries will start from queue 01 in the new batch
  // Older batches have priority over newer batches
  Future<bool> resetQueue({String? purpose, String? department}) async {
    try {
      print('Cut-Off / Reset triggered - Purpose: $purpose, Department: $department');

      // Increment batch number (this affects all departments globally)
      // Note: We don't delete any entries - they remain with their original batch numbers
      final newBatchNumber = await _incrementBatchNumber(
        resetReason: purpose ?? 'Manual reset'
      );

      print('✅ Queue reset completed. New batch number: $newBatchNumber');
      print('📝 Existing queue entries remain unchanged with their original batch numbers');
      print('🆕 New queue entries will start from queue 01 in batch $newBatchNumber');
  ...
  // Helper method to get next global queue number (across all departments)
  // Queue numbers are continuous across all departments
  Future<int> _getNextGlobalQueueNumber() async {
    try {
      // Get current batch number
      final currentBatch = await _getCurrentBatchNumber();
      
      // Get the maximum queue number in the current batch across ALL departments
      final response = await _supabase
          .from(SupabaseConfig.queueEntriesTable)
          .select(SupabaseConfig.queueNumberColumn)
          .eq(SupabaseConfig.batchNumberColumn, currentBatch)
          .order(SupabaseConfig.queueNumberColumn, ascending: false)
          .limit(1)
          .maybeSingle();

      if (response != null && response[SupabaseConfig.queueNumberColumn] != null) {
        final queueNumber = response[SupabaseConfig.queueNumberColumn] as int;
        return queueNumber + 1;
      }
      return 1; // Start from 1 if no entries exist in current batch
    } catch (e) {
      print('Error getting next global queue number: $e');
      return 1; // Start from 1 on error
    }
  }

  // Get current batch number from settings
  Future<int> _getCurrentBatchNumber() async {
    try {
      final response = await _supabase
          .from(SupabaseConfig.queueBatchSettingsTable)
          .select('current_batch_number')
          .eq('id', 'global')
          .maybeSingle();

      if (response != null && response['current_batch_number'] != null) {
        return response['current_batch_number'] as int;
      }
      // If settings don't exist, initialize with batch 1
      await _supabase
          .from(SupabaseConfig.queueBatchSettingsTable)
          .upsert({
            'id': 'global',
            'current_batch_number': 1,
            'last_reset_at': DateTime.now().toIso8601String(),
            'reset_reason': 'Initial setup'
          });
      return 1;
    } catch (e) {
      print('Error getting current batch number: $e');
      return 1; // Default to batch 1 on error
    }
  }

  // Increment batch number (for reset/cut-off)
  Future<int> _incrementBatchNumber({String? resetReason}) async {
    try {
      // Get current batch number
      final currentBatch = await _getCurrentBatchNumber();
      final newBatch = currentBatch + 1;
      ...
      await _supabase
          ...
          .upsert({
            'id': 'global',
            'current_batch_number': newBatch,
            'last_reset_at': DateTime.now().toIso8601String(),
            'reset_reason': resetReason ?? 'Manual reset'
          });

      print('✅ Batch number incremented from $currentBatch to $newBatch');
      return newBatch;
    } catch (e) {
      print('Error incrementing batch number: $e');
      rethrow;
    }
  }

  // Legacy method kept for backward compatibility (deprecated)
  // Use _getNextGlobalQueueNumber() instead
  @Deprecated('Use _getNextGlobalQueueNumber() for global numbering')
  Future<int> _getNextQueueNumberForDepartment(String department) async {
    return await _getNextGlobalQueueNumber();
  }
