...
class _AdminScreenState extends State<AdminScreen> {
  ...
  Future<void> _silentRefresh() async {
    ...
    try {
      ...
      newQueue.sort((a, b) {
        // Older batches first (lower batch number = older)
        final batchCompare = a.batchNumber.compareTo(b.batchNumber);
        if (batchCompare != 0) return batchCompare;
        
        // Priority users first (PWD/Senior) within same batch
        if (a.isPriority && !b.isPriority) return -1;
        if (!a.isPriority && b.isPriority) return 1;
        
        // Within same priority level and batch, sort by queue number (lower numbers first)
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              department == null
                  ? 'Queue cut-off completed. New entries will start from queue 01. Existing entries remain unchanged.'
                  : 'Queue cut-off completed for ${_currentAdmin!.department}. New entries will start from queue 01.',
            ),
            ...
            children: [
              ...
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: const Text('Cut-Off / Reset'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.red,
                    foregroundColor: Colors.white,
                  ),
